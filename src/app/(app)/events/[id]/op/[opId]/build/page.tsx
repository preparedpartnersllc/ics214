...
export default function BuildOrgPage() {
  ...
  // Ops — groups first, then teams, then divisions
  ...
  // Personnel assignment (ops/planning/logistics/finance)
  ...
  async function assignCommandStaff() {
    ...
    // Command staff go on a special "command" team
  ...
  const sectionPositionMap: Record<SectionTab, any[]> = {
    command: COMMAND_STAFF_POSITIONS,
    agency: [],
    ops: OPERATIONS_POSITIONS,
    planning: PLANNING_POSITIONS,
    logistics: LOGISTICS_POSITIONS,
    finance: FINANCE_POSITIONS,
  }

  const sectionAssignments = (tab: SectionTab) => {
    const positions = sectionPositionMap[tab].map(p => p.value)
    if (tab === 'command') return assignments.filter(a => positions.includes(a.ics_position) && a.ics_position !== 'agency_representative')
    if (tab === 'agency') return assignments.filter(a => a.ics_position === 'agency_representative')
    return assignments.filter(a => positions.includes(a.ics_position))
  }

  const cmdTeamId = teams.find(t => t.name === '__command__')?.id
  ...
  return (
    <div className="min-h-screen bg-zinc-950 px-4 py-8 max-w-2xl mx-auto">
      ...
      {/* Create profile button — available on all tabs */}
      ...
      {activeTab === 'command' && (
        <div className="space-y-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider mb-4">Command Staff</p>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
          ...
          <FormField label="Agency / Organization">
            <input type="text" className="input" value={agencyOrg}
              placeholder="e.g. Michigan State Police"
              onChange={e => setAgencyOrg(e.target.value)} />
          </FormField>
      ...
      {activeTab === 'ops' && (
        <div className="space-y-4">
                ...
                {opsTeams.map(t => {
                  ...
                  return (
                    <div key={t.id} className="flex items-center justify-between py-1.5 px-3 bg-zinc-800 rounded-lg">
                      <span className="text-sm text-zinc-200">{t.name}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider mb-3">Step 5 — Assign Personnel</p>
            <div className="space-y-3">
