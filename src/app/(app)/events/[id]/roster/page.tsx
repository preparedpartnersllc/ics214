...
export default function RosterPage() {
  ...
  async function loadOp(opId: string) {
    ...
    const [profResult, entryMap] = await Promise.all([
      userIds.length > 0
  ...
  function PersonRow({ a, showUnit = false }: { a: any; showUnit?: boolean }) {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-2.5 group">
        ...
        {/* Call button — only when phone exists */}
        {phone && (
          <a
            href={`tel:${phone}`}
            onClick={e => e.stopPropagation()}
            className="flex-shrink-0 text-[#374151] hover:text-[#22C55E] opacity-0 group-hover:opacity-100 sm:opacity-100 transition-colors p-1 rounded"
            title={`Call ${name}`}
          >
